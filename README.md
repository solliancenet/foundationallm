...
Please refer to the [docs](./docs/index.md) folder for documentation on this project.
...
Vectors are generated in two Change Feed handlers contained in the `FoundationaLLM.Core` project which monitor the `customer` and `products` containers. Anytime a new document is inserted into either of these containers, the Change Feed handler will generate a vector and add the document and its embedding to the `vector-index` Cognitive Search index.

You can see this at work when running locally by setting a break point on [GenericChangeFeedHandler() function](https://github.com/solliancenet/foundationallm/blob/main/src/Core/Services/CosmosDbService.cs#L115).
...
We're finally at the most critical part of this entire solution, generating a chat completion from Azure OpenAI using one of its [GPT models](https://platform.openai.com/docs/guides/gpt) wherein the Azure OpenAI service will take in all of the data we've gathered up to this point, then generate a response or completion which the user will see. All of this happens in the [SemanticKernelOrchestrationService](https://github.com/solliancenet/foundationallm/blob/main/src/Core/Services/SemanticKernelOrchestrationService.cs) in the [GetResponse()](https://github.com/solliancenet/foundationallm/blob/main/src/Core/Services/SemanticKernelOrchestrationService.cs#L124) function. 
...
The last part is to save the results of both our user prompt and completion as well as the amount of tokens used. All of the conversational history and the amount of tokens used in each prompt and completion is stored in the completions collection in the Azure Cosmos DB database in this solution. The call to the service is made by another function within our ChatService called, [AddPromptCompletionMessagesAsync()](https://github.com/solliancenet/foundationallm/blob/main/src/Core/Services/ChatService.cs#L187). This function creates two new [Message](https://github.com/solliancenet/foundationallm/blob/main/src/Core/Models/Chat/Message.cs) objects and stores them in a local cache of all the Sessions and Messages for the application. It then adds up all of the tokens used and saves it to the Session object which keeps a running total for the entire [Session](https://github.com/solliancenet/foundationallm/blob/main/src/Core/Models/Chat/Session.cs).

The data is then saved in the [UpsertSessionBatchAsync()](https://github.com/solliancenet/foundationallm/blob/main/src/Core/Services/CosmosDbService.cs#L350) function in the Cosmos DB database. This function creates a new transaction then updates the Session document and inserts two new Message documents into the completions collection.
