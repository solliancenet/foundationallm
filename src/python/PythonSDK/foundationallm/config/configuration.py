...
class Configuration():    
    def __init__(self):
        """Init"""
        vault_url = os.environ['foundationallm-keyvault-name'] # f"https://{self.keyvault_name}.vault.azure.net"
        credential=DefaultAzureCredential()
        secret_client = SecretClient(vault_url=vault_url, credential=credential)
        app_config_secret_name = os.environ.get("foundationallm-app-configuration-secret-name")
        connection_string = secret_client.get_secret(name=app_config_secret_name)
    ...
    def __get_config_with_retry(self, name):
        try:
            return self.__config.get_secret(name).value
        except RetryError:
            pass