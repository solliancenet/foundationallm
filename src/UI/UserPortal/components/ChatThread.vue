...
export default {
	...
	methods: {
		async rateMessage(message, rating) {
			...
			const data = await $fetch(`https://solliance-data-copilot-api.azurewebsites.net/sessions/${message.sessionId}/message/${message.id}/rate${message.rating !== null ? '?rating=' + message.rating : ''}`, {
				method: 'POST'
			});
			console.log(data);
		},
