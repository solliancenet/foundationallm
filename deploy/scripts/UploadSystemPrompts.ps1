...
az storage azcopy blob upload -c agents --account-name $storageAccount -s "./system-prompt/*" --recursive --only-show-errors
...
az storage azcopy blob upload -c foundationallm-source --account-name $storageAccount -s "./data-sources/*" --recursive --only-show-errors
